...
function createFileItem(fileData) {
    ...
    // Get file extension
    const fileExtension = fileData.fileName ? fileData.fileName.split('.').pop().toUpperCase() : 'FILE';
    
    // Format file size - handle both string and number formats
    const fileSize = formatFileSize(fileData.fileSize);
    
    // Format upload date
    let uploadDate = 'Unknown';
    if (fileData.uploadedAt && fileData.uploadedAt.seconds) {
        uploadDate = new Date(fileData.uploadedAt.seconds * 1000).toLocaleDateString();
    }
    ...
    fileItem.innerHTML = `
        <div class="file-info">
            ...
            <div class="file-details">
                <h3>${fileData.fileName || 'Untitled'}</h3>
...
function formatFileSize(size) {
    // If size is already a string (like "1.5 MB"), return as is
    if (typeof size === 'string' && size.trim() !== '') {
        return size;
    }
    
    // If size is a number (bytes), convert it
